...
let rec map = (f, seq, ()) =>
  ...
  | [@implicit_arity] Cons(x, next) =>
    [@implicit_arity] Cons(f(x), map(f, next))
  };
...
let rec filter_map = (f, seq, ()) =>
  ...
  | [@implicit_arity] Cons(x, next) =>
    ...
    | Some(y) => [@implicit_arity] Cons(y, filter_map(f, next))
    }
  };
...
let rec filter = (f, seq, ()) =>
  ...
  | [@implicit_arity] Cons(x, next) =>
    if (f(x)) {
      [@implicit_arity] Cons(x, filter(f, next));
    } else {
      filter(f, next, ());
    }
  };
...
let rec flat_map = (f, seq, ()) =>
  switch (seq()) {
  | Nil => Nil
  | [@implicit_arity] Cons(x, next) => flat_map_app(f, f(x), next, ())
  }
...
and flat_map_app = (f, seq, tail, ()) =>
  ...
  | Nil => flat_map(f, tail, ())
  | [@implicit_arity] Cons(x, next) =>
    [@implicit_arity] Cons(x, flat_map_app(f, next, tail))
  };
